...
int cnt=0, cnt2=0, mode=0;
int msec=0, sec=0;
int res_cnt=1; // 기본 탈수 1회
int i=0;
...
void bgm(){
    TCCR1A=0x40; TCCR1B=9; TCCR1C=0x80; OCR1A=0;
        ...
        sound(4,5);    //파
        delay_ms(1000);
        // PORTB=0xff;      //버저 포트 OFF     0b11101111
...
interrupt [EXT_INT0] void ext_int0_isr(void) // 전원
{
range=getEcho();
if(range<20) {cnt++;} // 일정거리 안에 들어와야만 전원이 켜진다.
if(cnt>=2) {cnt=0;}
delay_ms(55);
}

interrupt [EXT_INT1] void ext_int1_isr(void) // 모드 선택
{
if(cnt==0){OCR0=0;}
if(cnt==1){ // 전원버튼이 눌렀을 때에만 동작
    mode++;
    if(mode>3) mode=0;    
}
delay_ms(55);
...
{
if(cnt==1){res_cnt++;} // 전원 버튼이 눌려진 상태에서만 동작
delay_ms(50);
}
...
while (1)
    ...
    if(cnt%2==1){
       ...
       if(pause==1)
          ...
          if(mode==1){setak(); wash(); dewater();bgm(); }
          if(mode==2){wash();dewater();bgm();}
          if(mode==3){dewater();bgm();}
          }
       }
    }
}